...
def _repair(p):
    ...
    tr,ev=p.get('trainRowIds'),p.get('evalRowIds'); isolated=isinstance(tr,list) and isinstance(ev,list) and bool(tr) and bool(ev) and all(isinstance(x,str) and x for x in tr+ev) and len(tr)==len(set(tr)) and len(ev)==len(set(ev)) and not set(tr)&set(ev)
